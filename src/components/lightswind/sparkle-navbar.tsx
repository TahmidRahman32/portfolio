...
interface SparkleNavbarProps {
   ...
   items: string[];
   /**
    * An array of strings representing the routes for each menu item.
    * Should match the order of the items array.
   ...
   color?: string;
   /**
    * Optional: External control for active index
    */
   activeIndex?: number;
   /**
...
/**
 * A reusable navigation menu component with a dynamic, animated active state indicator and routing support.
...
const SparkleNavbar: React.FC<SparkleNavbarProps> = ({ items, routes, color = "#00fffc", activeIndex: externalActiveIndex, onItemClick }) => {
   ...
   // Update active index when route changes externally
   useLayoutEffect(() => {
   ...
   // Handler for button clicks, which triggers the animation and routing.
         ...
         keyframes: [
            ...
            },
            {
               ...
               onComplete: () => {
                  ...
                  // with the new active item (only if not externally controlled)
                  if (externalActiveIndex === undefined) {
