...
const CarouselItemCard: React.FC<CarouselItemProps> = ({ chain, side }) => {
   ...
   const IconOrLogo = (
      ...
      >
         {logo ? <img src={logo} alt={`${name} logo`} className="size-8 rounded-full object-cover" /> : <FallbackIcon className="size-8 text-background" />}
      </div>
   );
   ...
   return (
      ...
      >
         <div
            ...
            container mx-auto px-4 md:px-8 gap-12 justify-center items-center bg-input/50 py-16 rounded-2xl"
         >
               ...
               {currentItem && (
                  <div className="flex flex-col items-center justify-center gap-0 mt-4">
                     <div className="p-2 bg-foreground rounded-full">
                        {currentItem.logo ? <img src={currentItem.logo} alt={`${currentItem.name} logo`} className="size-12 rounded-full object-cover" /> : <currentItem.icon className="size-8 text-background" />}
                     </div>
                  ...
                  {showDropdown && filteredItems.length > 0 && (
                     ...
                     >
                        {filteredItems.slice(0, 10).map((chain) => (
                           ...
                           >
                              {chain.logo ? <img src={chain.logo} alt={`${chain.name} logo`} className="size-6 rounded-full object-cover" /> : <chain.icon size={24} className="text-primary" />}
